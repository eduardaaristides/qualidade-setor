...
export default function About() {
  ...
  async function addEvento() {
    const novo = {
      imagem: "", // Começa vazio, usuário fará upload
      titulo: "Novo título",
      texto: "Novo texto",
    };
  ...
  // 🖼️ Upload de imagem
  async function handleFileUpload(id: string, ev: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const data = await res.json();
      updateEvento(id, { imagem: data.url });
    } catch (err) {
              ...
              {eventos.map((e) => (
                ...
                >
                  ...
                  <input
                    type="file"
                    accept="image/*"
                    onChange={(ev) => handleFileUpload(e.id, ev)}
